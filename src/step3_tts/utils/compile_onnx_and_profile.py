"""Compile W8A16 QDQ Models with --target_runtime onnx and Profile on Samsung Galaxy S24 Ultra.

Target Runtime: ONNX (ONNXRuntime QNN EP)
"""
...
def main():
    ...
    device = devices[0]
    print(f"🚀 Submitting 4 W8A16 COMPILE (--target_runtime onnx) & PROFILE Jobs to: {device.name}")
    print("=" * 80)
    ...
    for name, quant_job_id in QUANT_JOBS.items():
        ...
        print(f" • [Step 1/2] Submitting COMPILE Job (--target_runtime onnx) for [{name:<25}]...")
        ...
        print(f" • [Step 2/2] Submitting PROFILE Job for [{name:<25}]...")
    ...
    print("=" * 80)
    print(" ✅ ALL 4 W8A16 ONNX RUNTIME COMPILE & PROFILE JOBS SUBMITTED!")
    print("=" * 80)
