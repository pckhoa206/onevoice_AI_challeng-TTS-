...
Exact Verified Input Specs:
  1. vocoder_w8a16            (latent: 1 x 144 x 64 float32)
  2. duration_predictor_w8a16 (text_ids: 1 x 64 int64, style_dp: 1 x 8 x 16 float32, text_mask: 1 x 1 x 64 float32)
  3. text_encoder_w8a16       (text_ids: 1 x 64 int64, style_ttl: 1 x 8 x 16 float32, text_mask: 1 x 1 x 64 float32)
  4. vector_estimator_w8a16   (noisy_latent: 1 x 144 x 64 float32, text_emb: 1 x 256 x 64 float32, style_ttl: 1 x 8 x 16 float32, latent_mask: 1 x 1 x 64 float32, text_mask: 1 x 1 x 64 float32, current_step: 1 float32, total_step: 1 float32)
...
def main():
    ...
    device = devices[0]
    print(f"🚀 Submitting 4 EXACT INFERENCE Jobs to Qualcomm Cloud Device: {device.name}")
    print("=" * 80)

    # Common shapes
    text_ids_64 = np.ones((1, 64), dtype=np.int64)
    text_mask_64 = np.ones((1, 1, 64), dtype=np.float32)
    style_dp_data = np.random.randn(1, 8, 16).astype(np.float32)
    style_ttl_data = np.random.randn(1, 8, 16).astype(np.float32)
    ...
    inf_voc.set_name("[OFFICIAL_FINAL] Supertonic3_vocoder_w8a16")
    ...
    inf_dp = hub.submit_inference_job(
        model=model_dp,
        inputs=dict(text_ids=[text_ids_64], style_dp=[style_dp_data], text_mask=[text_mask_64]),
        device=device
    )
    inf_dp.set_name("[OFFICIAL_FINAL] Supertonic3_duration_predictor_w8a16")
    ...
    inf_te = hub.submit_inference_job(
        model=model_te,
        inputs=dict(text_ids=[text_ids_64], style_ttl=[style_ttl_data], text_mask=[text_mask_64]),
        device=device
    )
    inf_te.set_name("[OFFICIAL_FINAL] Supertonic3_text_encoder_w8a16")
    ...
    inf_ve.set_name("[OFFICIAL_FINAL] Supertonic3_vector_estimator_w8a16")
    print(f"   --> Inference Job ID: {inf_ve.job_id}")

    print("=" * 80)
    print(" ✅ ALL 4 EXACT INFERENCE JOBS SUBMITTED SUCCESSFULLY!")
    print("=" * 80)
