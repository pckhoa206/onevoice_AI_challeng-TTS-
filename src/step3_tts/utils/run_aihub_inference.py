...
Configured with exact frame-bound static shapes:
  1. vocoder_w8a16            (latent: 1 x 144 x 100 float32)
  2. duration_predictor_w8a16 (text_ids: 1 x 100 int32, style_ttl: 1 x 50 x 256 float32, speed: 1 float32)
  3. text_encoder_w8a16       (text_ids: 1 x 100 int32, style_ttl: 1 x 50 x 256 float32, speed: 1 float32)
  4. vector_estimator_w8a16   (sample: 1 x 144 x 100 float32, timestep: 1 float32, text_emb: 1 x 256 x 100 float32, style_ttl: 1 x 50 x 256 float32)
...
def main():
    ...
    inf_voc.set_name("[OFFICIAL_INFERENCE] Supertonic3_vocoder_w8a16_SUCCESS")
    ...
    text_ids_100 = np.ones((1, 100), dtype=np.int32)
    ...
    inf_dp = hub.submit_inference_job(
        model=model_dp,
        inputs=dict(text_ids=[text_ids_100], style_ttl=[style_ttl_data], speed=[speed_data]),
        device=device
    )
    inf_dp.set_name("[OFFICIAL_INFERENCE] Supertonic3_duration_predictor_w8a16_SUCCESS")
    ...
    inf_te.set_name("[OFFICIAL_INFERENCE] Supertonic3_text_encoder_w8a16_SUCCESS")
    ...
    sample_100 = np.random.randn(1, 144, 100).astype(np.float32)
    ...
    inf_ve.set_name("[OFFICIAL_INFERENCE] Supertonic3_vector_estimator_w8a16_SUCCESS")
    print(f"   --> Inference Job ID: {inf_ve.job_id}")

    print("=" * 80)
    print(" ✅ ALL 4 INFERENCE JOBS SUCCESSFULLY PROCESSED ON QUALCOMM CLOUD DEVICE!")
    print("=" * 80)
