"""Qualcomm AI Hub Static ONNX QNN Execution Provider Compilation & Live Inference Pipeline.

Submits static ONNX compilation jobs (--target_runtime onnx) for all 4 Supertonic 3 submodels,
producing NPU-accelerated ONNX Runtime QNN models and running live hardware inference on Samsung Galaxy S24 Ultra.
"""
import os
import sys
import time
...
def run_pipeline():
    _ensure_utf8_stdout()
    print("=" * 80)
    print(" 🚀 QUALCOMM AI HUB — STATIC ONNX QNN EP COMPILATION & LIVE INFERENCE")
    ...
    for cfg in SUBMODEL_CONFIGS:
        ...
        print(f"\n[1/2] Compiling Submodel: '{name}'...")
        compile_job = hub.submit_compile_job(
            model=onnx_path,
            device=device,
            input_specs=specs,
            options="--target_runtime onnx",
            name=f"{name}_static_onnx_qnn_ep",
        )
    ...
    # Step 2: Submit Live Hardware Inference Jobs for all successfully compiled models
    print("\n" + "=" * 80)
    print(" 📊 SUBMITTING LIVE HARDWARE INFERENCE JOBS ON SAMSUNG GALAXY S24 ULTRA")
    print("=" * 80)
    ...
    for name, (target_model, sample_inputs) in compiled_models.items():
        ...
        inf_job = hub.submit_inference_job(
            ...
            name=f"{name}_live_qnn_ep_inference",
        )
    ...
    print("\n" + "=" * 80)
    print(" 🏁 WAITING FOR LIVE HARDWARE INFERENCE RESULTS & RETRIEVING TENSORS")
    print("=" * 80)
